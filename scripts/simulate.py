...
def add_fog(image, depth, fog_intensity=0.005, fog_color=(200, 200, 200)):
    """
    Adds fog to an RGB image based on depth information.

    Parameters:
        image (numpy array): The RGB image to which fog will be added.
        depth (numpy array): The depth map, where larger values mean farther away.
        fog_intensity (float): Controls the intensity of the fog.
        fog_color (tuple): The color of the fog in RGB.

    Returns:
        numpy array: The RGB image with fog added.
    """
    # Normalize the depth map to the range [0, 1]
    # normalized_depth = cv2.normalize(depth, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    normalized_depth = depth
    print(depth.shape)

    # Calculate fog factor based on depth (more depth -> more fog)
    fog_factor = 1 - np.exp(-fog_intensity * normalized_depth)
    fog_factor = fog_factor[
        ..., np.newaxis
    ]  # Expand dimensions to match image channels for broadcasting

    # Create a fog image based on the fog color
    fog_image = np.full_like(image, fog_color, dtype=np.uint8)
    kernel_size = 5
    kernel = np.ones((kernel_size, kernel_size), np.float32) / (
        kernel_size * kernel_size
    )
    fog_image = cv2.filter2D(fog_image, -1, kernel)

    # Blend the original image with the fog image based on the fog factor
    foggy_image = (
        image.astype(np.float32) * (1 - fog_factor)
        + fog_image.astype(np.float32) * fog_factor
    ).astype(np.uint8)
    ...
    with ExitStack() as stack:
        ...
        with progress:
            for camera_idx in progress.track(range(cameras.size), description=""):
                objdata = pipeline.datamanager.train_dataset.metadata["obj_info"][
                    camera_idx
                ].to(pipeline.model.object_meta["obj_metadata"].device)
                obj_metadata = pipeline.datamanager.eval_dataset.metadata[
                    "obj_metadata"
                ].to(pipeline.model.object_meta["obj_metadata"].device)
                ...
                camera_ray_bundle.metadata["directions_norm"] = (
                    camera_ray_bundle.metadata["directions_norm"].reshape(
                ...
                camera_ray_bundle.metadata["object_rays_info"] = batch_obj_dyn.reshape(
                    batch_obj_dyn.shape[0] * batch_obj_dyn.shape[1],
                    batch_obj_dyn.shape[2] * batch_obj_dyn.shape[3],
                )
                ...
                with torch.no_grad():
                    outputs = pipeline.model.get_outputs_for_camera_ray_bundle_render(
                        camera_ray_bundle
                    )
                render_image = []
                render_depth = []
                for rendered_output_name in rendered_output_names:
                    ...
                    if rendered_output_name == "depth":
                        depth = outputs["depth"]
                        filepath = (
                            pipeline.datamanager.train_dataparser_outputs.metadata[
                                "depth_filenames"
                            ][camera_idx]
                        )
                        scale_factor = (
                        ...
                        depth_mask = (
                            torch.abs(depth_img_gt / scale_factor - 65535) > 1e-6
                        )
                        ...
                        render_depth.append(outputs["depth"])
                    if rendered_output_name == "semantics":
                        semantic_labels = torch.argmax(
                            torch.nn.functional.softmax(outputs["semantics"], dim=-1),
                            dim=-1,
                        )
                        colormap = (
                ...
                if output_format == "images":
                    (output_image_dir / "rgb").mkdir(parents=True, exist_ok=True)
                    (output_image_dir / "depth").mkdir(parents=True, exist_ok=True)
                    (output_image_dir / "fog").mkdir(parents=True, exist_ok=True)
                    media.write_image(
                        output_image_dir / "rgb" / f"{camera_idx:05d}.png",
                        render_image[-1],
                    )
                    media.write_image(
                        output_image_dir / "depth" / f"{camera_idx:05d}.png",
                        colormaps.apply_depth_colormap(render_depth[-1].cpu()),
                    )
                    foggy_image = add_fog(
                        cv2.imread(
                            f"/DATA_EDS2/chenjt2305/datasets/vkitti-4/Scene20/clone/frames/rgb/Camera_0/rgb_{camera_idx+260:05d}.jpg"
                        ),
                        # np.array(render_depth[-1][:, :, 0].cpu()),
                        cv2.imread(
                            f"/DATA_EDS2/chenjt2305/datasets/vkitti-4/Scene20/clone/frames/depth/Camera_0/depth_{camera_idx+260:05d}.png",
                            cv2.IMREAD_GRAYSCALE,
                        ),
                        fog_intensity=0.2,
                    )
                    # media.write_image(
                    #     output_image_dir / "fog" / f"{camera_idx:05d}.png",
                    #     foggy_image,
                    # )
...
# python scripts/cicai_render.py --load-config /data22/DISCOVER_summer2023/huangyx/NSG-Studio/outputs/vkitti20-260-300-w-secondpass/vkitti-full-carnerf/2024-03-07_074944/config.yml --output-path renders/ref-mars/20_260_300_w/
