...
from mars.fields_components.fields_head import FieldHeadNames, ReflectanceFieldHead
...
class NerfactoField(Field):
    ...
    def __init__(
        ...
        self.predict_reflectance = predict_reflectance
        ...
        if self.predict_reflectance:
            self.mlp_predict_reflectance = MLP(
                in_dim=self.position_encoding.get_out_dim() + self.geo_feat_dim,
                num_layers=3,
                layer_width=64,
                out_dim=1,
                activation=nn.ReLU(),
            ...
            self.field_head_reflectance = ReflectanceFieldHead(
                in_dim=self.mlp_predict_reflectance.get_out_dim()
            )
    ...
    def get_outputs(
        ...
        if self.use_obj_feat:
            if self.training:
                ...
                obj_feat = torch.ones(
                    (*directions.shape[:-1], self.obj_feat_dim),
        ...
        if self.use_obj_feat:
            h = torch.cat([h, obj_feat.view(-1, self.obj_feat_dim)], dim=-1)
        rgb = self.mlp_head(h).view(*outputs_shape, -1).to(directions)
        outputs.update({FieldHeadNames.RGB: rgb})

        if self.predict_reflectance:
            # reflectance_out = (
            #     self.mlp_predict_reflectance(h).view(*outputs_shape, -1).to(directions)
            # )
            # outputs["reflectance"] = self.field_head_reflectance(reflectance_out)
            reflectance_out = (
                self.mlp_predict_reflectance(pred_normals_inp)
                .view(*outputs_shape, -1)
                .to(directions)
            )
            outputs["reflectance"] = reflectance_out
