    ...
    def train(self) -> None:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                if self.config.pipeline.datamanager.dataparser.start_later and step == self.config.start_second_pass:
                    self.config.pipeline.model.background_model.predict_reflectance = True
                    self.config.pipeline.model.object_model_template.predict_reflectance = True
                    self.config.pipeline.model.second_pass = True
                    self.config.load_dir = self.base_dir / "nerfstudio_models/"
                    self.config.load_checkpoint = (step // self.config.steps_per_save) * self.config.steps_per_save
                    self.setup()
